JsUnitTest.Unit.DOMLogger = function(element) {
  this.element = JsUnitTest.$(element);
  if (this.element) this._createLogTable();
};
  
JsUnitTest.Unit.DOMLogger.prototype.start = function(testName) {
...
JsUnitTest.Unit.DOMLogger.prototype.setStatus = function(status) {
...
JsUnitTest.Unit.DOMLogger.prototype.finish = function(status, summary) {
  if (!this.element) return;
  this.setStatus(status);
  this.message(summary);
};
  
JsUnitTest.Unit.DOMLogger.prototype.message = function(message) {
  if (!this.element) return;
...
JsUnitTest.Unit.DOMLogger.prototype.summary = function(summary) {
  if (!this.element) return;
  var div = this.element.getElementsByTagName('div')[0];
  div.innerHTML = this._toHTML(summary);
};
  
JsUnitTest.Unit.DOMLogger.prototype.getLastLogLine = function() {
  var tbody = this.element.getElementsByTagName('tbody')[0];
  var loglines = tbody.getElementsByTagName('tr');
  return loglines[loglines.length - 1];
};
  
JsUnitTest.Unit.DOMLogger.prototype.getMessageCell = function() {
  var logline = this.getLastLogLine();
  return logline.getElementsByTagName('td')[2];
};
  
JsUnitTest.Unit.DOMLogger.prototype._createLogTable = function() {
...
JsUnitTest.Unit.DOMLogger.prototype.appendActionButtons = function(actions) {
  // actions = $H(actions);
...
JsUnitTest.Unit.DOMLogger.prototype._toHTML = function(txt) {
